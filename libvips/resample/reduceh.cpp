	...
	void *a, void *b, gboolean *stop )
{
	...
	for( int y = 0; y < r->height; y++ ) { 
		...
		double *cxf = reduceh->coef_f ?
			reduceh->coef_f + r->left * reduceh->coef_stride :
			NULL;
		int *cxi = reduceh->coef_i ?
			reduceh->coef_i + r->left * reduceh->coef_stride :
			NULL;
		...
		for (int x = 0; x < r->width; x++) {
			...
			if (cxf)
				cxf += reduceh->coef_stride;
			if (cxi)
				cxi += reduceh->coef_stride;
			bounds += 2;
		}
	}
	...
	void *a, void *b, gboolean *stop)
{
	...
	short *cxs = &reduceh->coef_s[r->left * reduceh->coef_stride];
	int *bounds = &reduceh->bounds[r->left * 2];
	...
	for (int y = 0; y < r->height; y++) {
		...
		reduceh_uchar_simd(q, p0, r->width, in->Bands,
			cxs, reduceh->coef_stride, bounds);
	}
...
vips_reduceh_build( VipsObject *object )
{
	...
	reduceh->coef_stride = reduceh->n_point;

#if HAVE_SIMD
	if (in->BandFmt == VIPS_FORMAT_UCHAR &&
		(in->Bands == 4 || in->Bands == 3)) {

		/* Make coef_stride even to make all matrices 4-bytes aligned
	         */
		if (reduceh->coef_stride % 2)
			reduceh->coef_stride++;

		reduceh->coef_s = VIPS_ARRAY(object, width * reduceh->coef_stride, short);
		if (!reduceh->coef_s)
			return -1;
	...
	if (in->BandFmt == VIPS_FORMAT_UCHAR ||
		...
		reduceh->coef_i = VIPS_ARRAY(object, width * reduceh->coef_stride, int);
		if (!reduceh->coef_i)
			return (-1);
	} else {
		reduceh->coef_f = VIPS_ARRAY(object, width * reduceh->coef_stride, double);
		if (!reduceh->coef_f)
			return -1;
	}
