	...
	void *a, void *b, gboolean *stop )
{
	...
	for( int y = 0; y < r->height; y++ ) { 
		...
		double *cxf = reduceh->coef_f + r->left * reduceh->n_point;
		int *cxi = reduceh->coef_i ?
			reduceh->coef_i + r->left * reduceh->n_point : NULL;
...
vips_reduceh_build( VipsObject *object )
{
	...
	for( int x = 0; x < width; x++ ) {
		...
		if( right > in->Xsize ) right = in->Xsize;

		vips_reduce_make_mask( coef_f, reduceh->kernel,
			reduceh->hshrink, X - ix, start, right - left );
	...
	if( in->BandFmt == VIPS_FORMAT_UCHAR &&
		...
		reduceh->coef_s = VIPS_ARRAY(
			object, width * reduceh->n_point, short );
		if( !reduceh->coef_s )
			return( -1 );

		for( int i = 0; i < width * reduceh->n_point; i++ )
			reduceh->coef_s[i] = reduceh->coef_f[i] *
	...
	if( in->BandFmt == VIPS_FORMAT_UCHAR ||
		...
		reduceh->coef_i = VIPS_ARRAY(
			object, width * reduceh->n_point, int );
		if( !reduceh->coef_i )
			return( -1 );

		for( int i = 0; i < width * reduceh->n_point; i++ )
			reduceh->coef_i[i] = reduceh->coef_f[i] *
				VIPS_INTERPOLATE_SCALE;
	}
