	...
	void *a, void *b, gboolean *stop )
{
	...
	double *restrict cyf = reducev->coef_f ?
		reducev->coef_f + r->top * reducev->coef_stride :
		NULL;
	int *restrict cyi = reducev->coef_i ?
		reducev->coef_i + r->top * reducev->coef_stride :
		NULL;
	...
	for (int y = 0; y < r->height; y++) {
		...
		if (cyf)
			cyf += reducev->coef_stride;
		if (cyi)
			cyi += reducev->coef_stride;
		bounds += 2;
	}
	...
	void *a, void *b, gboolean *stop)
{
	...
	short *restrict cys = &reducev->coef_s[r->top * reducev->coef_stride];
	int *restrict bounds = &reducev->bounds[r->top * 2];
...
#endif /*!HAVE_SIMD*/
	{
		reducev->coef_stride = reducev->n_point;
		...
		if (in->BandFmt == VIPS_FORMAT_UCHAR) {
			 ...
			 */
			if (reducev->coef_stride % 2)
				reducev->coef_stride++;

			reducev->coef_s =
				VIPS_ARRAY(NULL, height * reducev->coef_stride, short);
			if (!reducev->coef_s)
				return -1;
		...
		if (in->BandFmt == VIPS_FORMAT_UCHAR ||
			...
			reducev->coef_i =
				VIPS_ARRAY(NULL, height * reducev->coef_stride, int);
			if (!reducev->coef_i)
				return (-1);
		} else {
			reducev->coef_f =
				VIPS_ARRAY(NULL, height * reducev->coef_stride, double);
			if (!reducev->coef_f)
				return -1;
		}
