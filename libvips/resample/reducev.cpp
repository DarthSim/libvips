	...
	void *a, void *b, gboolean *stop )
{
	...
	double * restrict cyf = reducev->coef_f + r->top * reducev->n_point;
	int * restrict cyi = reducev->coef_i ?
		reducev->coef_i + r->top * reducev->n_point : NULL;
	...
	if( generate == vips_reducev_gen ) {
		...
		*/
		for( int y = 0; y < height; y++ ) {
			...
			if( bottom > in->Ysize ) bottom = in->Ysize;

			vips_reduce_make_mask( coef_f, reducev->kernel,
				reducev->vshrink, Y - iy, start, bottom - top );
...
#if HAVE_SIMD
		if( in->BandFmt == VIPS_FORMAT_UCHAR ) {
			reducev->coef_s = VIPS_ARRAY(
				NULL, height * reducev->n_point, short );
			if( !reducev->coef_s )
				return( -1 );

			for( int i = 0; i < height * reducev->n_point; i++ )
				reducev->coef_s[i] = reducev->coef_f[i] *
		...
		if( in->BandFmt == VIPS_FORMAT_UCHAR ||
			...
			reducev->coef_i = VIPS_ARRAY(
				NULL, height * reducev->n_point, int );
			if( !reducev->coef_i )
				return( -1 );

			for( int i = 0; i < height * reducev->n_point; i++ )
				reducev->coef_i[i] = reducev->coef_f[i] *
					VIPS_INTERPOLATE_SCALE;
		}
	}
